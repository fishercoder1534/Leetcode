    ...
    public static class Solution1 {
        ...
        public int minOperations(int[] nums) {
            ...
            for (int i = 0; i < nums.length; i++) {
                if (nums[i] == 0 && ops % 2 == 0) {
                    //this means after an even number of flipping, this number is (originally) a zero, so we need to flip it and all the numbers to its right
                    ops++;
                }
                if (nums[i] == 1 && ops % 2 == 1) {
                    //this means after an odd number of flipping prior to reaching this number and this number is a one, so it should have been flipped to become a zero, so we need to flip it
                    ops++;
                }
            }
            return ops;
