...
public class _912 {
    public static class Solution1 {
        ...
        private int[] sort(int[] nums, int left, int right) {
            // this condition keeps dividing the array until nums becomes one individual item and
            // then it goes back to the call stack
            if (left < right) {
        ...
        private void merge(int[] nums, int left, int mid, int right) {
            ...
            for (int i = 0; i < leftSize; i++) {
                // this index is key: it should be nums[left + i] as it should start from left
                // instead of zero
                leftHalf[i] = nums[left + i];
            }
            for (int i = 0; i < rightSize; i++) {
                // similarly, this index is key as well: it should be nums[mid + i + 1] instead of
                // starting from zero
