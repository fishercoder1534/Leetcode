...
public class _72 {
    ...
    public static class Solution2 {
        public int minDistance(String word1, String word2) {
            // using levenshtein Distance to find minimum transformation operations required from
            // word 1 to word 2
        ...
        private int levenshteinDistance(
                String s1, int s1Index, String s2, int s2Index, int[][] dp) {
            ...
            if (s1.charAt(s1Index) == s2.charAt(s2Index)) {
                ...
                dp[s1Index][s2Index] =
                        1 + Math.min(insertValue, Math.min(deleteValue, substituteValue));
            }
            return dp[s1Index][s2Index];
        }
    }
