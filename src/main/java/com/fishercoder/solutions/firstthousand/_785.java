...
public class _785 {
    public static class Solution1 {
        ...
        public boolean isBipartite(int[][] graph) {
            ...
            // BFS
            // 0 means never encountered before, 1 means we put this node into set A, 2 means we put
            // this node into set B
                ...
                if (graph[i].length != 0 && visited[i] == 0) {
                    ...
                    while (!queue.isEmpty()) {
                        ...
                        for (int node : graph[current]) {
                            if (visited[node] == 0) {
                                // if the current node is in set A (1), then we put its neighbor in
                                // set B (2), otherwise set A (1)
                    ...
                    while (!stack.isEmpty()) {
                        ...
                        for (int neighbor : graph[curr]) {
                            if (colors[neighbor] == 0) {
                                stack.push(neighbor);
                                // if the current node is red (1), then we color it to be blue (2),
                                // otherwise red (1)
                                colors[neighbor] = (colors[curr] == 1) ? 2 : 1;
                            } else if (colors[neighbor] == colors[curr]) {
                                // this means the two connected nodes have the same color, so this
                                // is not a bipartite
                                return false;
                            }
                        }
