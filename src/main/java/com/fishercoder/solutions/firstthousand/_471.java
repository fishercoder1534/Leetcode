        ...
        public String encode(String s) {
            ...
            for (int l = 0; l < s.length(); l++) {
                for (int i = 0; i < s.length() - l; i++) {
                    ...
                    // Checking if string length < 5. In that case, we know that encoding will not
                    // help.
                    if (j - i < 4) {
                        dp[i][j] = substr;
                    } else {
                        ...
                        // Loop for checking if string can itself found some pattern in it which
                        // could be repeated.
                        for (int k = 0; k < substr.length(); k++) {
                            ...
                            if (repeatStr != null
                                ...
                                String ss =
                                        substr.length() / repeatStr.length()
                                                + "["
                                                + dp[i][i + k]
                                                + "]";
