...
public class _426 {
    ...
    public static class Solution1 {
        ...
        private void dfs(Node node) {
            ...
            dfs(node.left);
            // we only need to assign head once, i.e. when it's not assigned, we'll assign the most
            // bottom left leaf node to head
            if (head == null) {
                head = node;
            }
            // if the tail is already assigned, which should be all of the cases except when it's
            // just finding the most bottom left leaf
