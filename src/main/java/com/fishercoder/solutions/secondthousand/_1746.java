...
public class _1746 {
    public static class Solution1 {
        ...
        public int maxSumAfterOperation(int[] nums) {
            int len = nums.length;
            // dp[i][0] means the sum of all elements in the subarray up to index i without any
            // number squared
            // dp[i][1] means the sum of all elements in the subarray up to index i with nums[i]
            // squared
            ...
            for (int i = 1; i < len; i++) {
                ...
                dp[i][1] =
                        Math.max(
                                nums[i] * nums[i],
                                Math.max(dp[i - 1][0] + nums[i] * nums[i], dp[i - 1][1] + nums[i]));
