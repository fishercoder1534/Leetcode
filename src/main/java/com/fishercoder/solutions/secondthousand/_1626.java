...
public class _1626 {
    public static class Solution1 {
        public int bestTeamScore(int[] scores, int[] ages) {
            ...
            // dp array is the max possible score up to this age, i.e. dp[2] means the max score up
            // to from age 0 up to age 2
            int[] dp = new int[len];
            dp[0] = players[0][1];
            for (int i = 1; i < len; i++) {
                int maxScoreUpToAgeI =
                        players[i][1]; // this is the max score possible on this age i alone
                for (int j = 0; j < i; j++) {
